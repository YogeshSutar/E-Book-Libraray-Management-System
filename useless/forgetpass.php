...
function sendMail($email, $randomCode,$reset_token)
{
    ...
    try {
        ...
        $mail->Username   = 'ganeshpswami184@gmail.com';                     //SMTP username
        $mail->Password   = 'Ganesh@41525';                               //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
    
        //Recipients
        $mail->setFrom('ganeshpswami184@gmail.com', 'knowledge library');
...
if($result)
{
    if(mysqli_num_rows($result)==1)
    {
        ...
        $query = "UPDATE `otpd` SET `otpid`='',``otp` ='$randomCode' WHERE `email`='$_POST[email]'  ";
