...
function sendMail($email, $randomCode,$reset_token)
{
    ...
    try {
        ...
        $mail->Username   = '';                     //SMTP write running Email ID username
        $mail->Password   = '';                     //SMTP Write a password of real Email Id 
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
    
        //Recipients
        $mail->setFrom('gmail@gamil.com', 'knowledge library');
...
{
$query="SELECT * FROM `idpass` WHERE `email`='$_POST[email]' ";
...
if($result)
{
    if(mysqli_num_rows($result)==1)
    {
        ...
        $query = "UPDATE `idpass` SET `resettoken`='$reset_token',`resettokenexpire`='$date',`sendotp`='$randomCode' WHERE `email`='$_POST[email]'  ";
        if(mysqli_query($con, $query) && sendMail($_POST['email'], $randomCode,$reset_token))
        {   
